...
function renderEntryQuickStats(totals) {
  ...
  const pricedRows = state.positions
    .map((position) => ({ position, metrics: calculatePosition(position) }))
    .filter((row) => Number.isFinite(row.metrics.currentValue) && row.metrics.currentValue > 0);

  if (!pricedRows.length || !totals.value) {
    dom.entryQuickStats.innerHTML = `
      <div class="quick-stat-head">
        <h3>快速概覽</h3>
        <span>等待報價</span>
      </div>
      <div class="quick-empty">新增或同步資產後，這裡會顯示加密貨幣、美股、台股的配置摘要。</div>
    `;
    return;
  }

  const categoryCards = ALLOCATION_CATEGORIES
    .map((category) => {
      const value = pricedRows
        .filter((row) => row.position.kind === category.kind)
        .reduce((sum, row) => sum + row.metrics.currentValue, 0);
      const percent = totals.value > 0 ? (value / totals.value) * 100 : 0;
      return `
        <div class="quick-category-card">
          <div>
            <span class="quick-dot" style="background:${category.color}"></span>
            <strong>${escapeHtml(category.label)}</strong>
          </div>
          <span class="quick-percent">${percent.toFixed(1)}%</span>
          <span class="quick-value ${sensitiveClass(value)}">${formatSensitiveCurrency(value)}</span>
          <span class="quick-bar"><span style="width:${Math.min(100, percent).toFixed(1)}%;background:${category.color}"></span></span>
        </div>
      `;
    })
    .join("");

  const largest = [...pricedRows].sort((a, b) => b.metrics.currentValue - a.metrics.currentValue)[0];
  const best = [...pricedRows]
    .filter((row) => Number.isFinite(row.metrics.profitPercent))
    .sort((a, b) => b.metrics.profitPercent - a.metrics.profitPercent)[0];
  const worst = [...pricedRows]
    .filter((row) => Number.isFinite(row.metrics.profitPercent))
    .sort((a, b) => a.metrics.profitPercent - b.metrics.profitPercent)[0];
  const largestPercent = largest && totals.value > 0 ? (largest.metrics.currentValue / totals.value) * 100 : NaN;

  dom.entryQuickStats.innerHTML = `
    <div class="quick-stat-head">
      <h3>快速概覽</h3>
      <span>${formatRefreshInterval(REFRESH_MS)}刷新</span>
    </div>
    ...
    <div class="quick-snapshot-grid">
      <div>
        <span>最大部位</span>
        <strong>${escapeHtml(largest?.position.symbol || "--")} ${Number.isFinite(largestPercent) ? largestPercent.toFixed(1) : "--"}%</strong>
      </div>
      <div>
        <span>表現最好</span>
        <strong class="${valueClass(best?.metrics.profitPercent)}">${best ? `${escapeHtml(best.position.symbol)} ${formatPercent(best.metrics.profitPercent)}` : "--"}</strong>
      </div>
      <div>
        <span>壓力最大</span>
        <strong class="${valueClass(worst?.metrics.profitPercent)}">${worst ? `${escapeHtml(worst.position.symbol)} ${formatPercent(worst.metrics.profitPercent)}` : "--"}</strong>
      </div>
    </div>
  `;
}
...
function registerServiceWorker() {
  ...
  navigator.serviceWorker.register("./service-worker-v35.js", { scope: "./" }).catch((error) => {
