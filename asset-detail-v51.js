...
async function fetchSecFundamentals(target) {
  ...
  let capex = annualConcept(gaap, ["PaymentsToAcquirePropertyPlantAndEquipment", "PaymentsToAcquireProductiveAssets"], (unit) => unit === "USD");
...
function parseSecTextCapexFacts(html) {
  ...
  const row = section.match(/Purchases of property and equipment(?: and intangible assets)?\s+(.{0,220}?)(?:Reimbursement|Investment|Net cash|Proceeds|Payments|Cash flows from financing activities)/i);
...
function parseInlineNumberList(text) {
  return [...String(text || "").matchAll(/\(?\s*\d[\d,]*\s*\)?/g)]
...
function isCapexInlineConcept(name) {
  return /:(PurchasesOfPropertyAndEquipmentAndIntangibleAssets|PaymentsToAcquirePropertyAndEquipment|PaymentsToAcquirePropertyPlantAndEquipment|PaymentsToAcquireProductiveAssets)$/i.test(String(name || ""));
}
