(function () {
  ...
  function calculateDividendSummaryV70(rows) {
  ...
  function restoreDividendPayerScroll(scrollTop) {
    const nextPayers = document.querySelector("#entryQuickStats .dividend-payers");
    if (!nextPayers) return;
    const maxScroll = Math.max(0, nextPayers.scrollHeight - nextPayers.clientHeight);
    const nextScrollTop = Math.min(Math.max(0, scrollTop), maxScroll);
    nextPayers.scrollTop = nextScrollTop;
    dividendPayerScrollTop = nextScrollTop;
  }

  function bindDividendPayerScrollMemory() {
    const nextPayers = document.querySelector("#entryQuickStats .dividend-payers");
    if (!nextPayers || nextPayers.__wealthtrackScrollMemory) return;
    nextPayers.__wealthtrackScrollMemory = true;
    nextPayers.addEventListener("scroll", () => {
      dividendPayerScrollTop = nextPayers.scrollTop;
    }, { passive: true });
  }

  function renderEntryQuickStatsV70() {
    const target = document.getElementById("entryQuickStats");
    if (!target) return;

    const currentPayers = target.querySelector(".dividend-payers");
    const previousScrollTop = currentPayers ? currentPayers.scrollTop : dividendPayerScrollTop;
    const dividendRows = getDividendRows();
    if (!dividendRows.length) return;

    const summary = calculateDividendSummaryV70(dividendRows);
    ...
    restoreDividendPayerScroll(previousScrollTop);
    requestAnimationFrame(() => restoreDividendPayerScroll(previousScrollTop));
  }

  try {
    const originalRender = render;
    if (!originalRender.__dividendV70) {
      render = function renderWithDividendV70() {
        originalRender();
        renderEntryQuickStatsV70();
      };
      render.__dividendV70 = true;
    }
  } catch (error) {
