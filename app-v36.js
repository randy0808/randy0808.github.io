...
function renderEntryQuickStats(totals) {
    ...
    .map((value, index) => {
      ...
      return `
        <div class="dividend-month-card ${value > 0 ? "has-dividend" : ""}">
          ...
          <strong class="${sensitiveClass(value)}">${value > 0 ? formatSensitiveCurrency(value) : "--"}</strong>
  ...
  const topPayers = summary.payers
    ...
    .map((payer) => `
      <div class="dividend-payer-row">
        ...
        <strong class="${sensitiveClass(payer.annualValue)}">${formatSensitiveCurrency(payer.annualValue)}</strong>
  ...
  dom.entryQuickStats.innerHTML = `
    ...
    <div class="dividend-summary-grid">
      <div>
        <span>年化股息</span>
        <strong class="${sensitiveClass(summary.annualTotal)}">${formatSensitiveCurrency(summary.annualTotal)}</strong>
      </div>
      <div>
        <span>月平均</span>
        <strong class="${sensitiveClass(summary.monthAverage)}">${formatSensitiveCurrency(summary.monthAverage)}</strong>
      </div>
      <div>
        <span>資料狀態</span>
    ...
    <div class="dividend-bottom-grid">
      ...
      <div class="dividend-note">
        ...
        <small>未扣海外稅、二代健保、匯費；資料每日更新一次。</small>
      </div>
    </div>
  `;
}
...
function calculateDividendSummary(rows) {
  ...
  for (const row of rows) {
    ...
    for (const [monthIndex, amountPerShare] of Object.entries(monthAmounts)) {
      ...
      const amount = Number(amountPerShare) * Number(row.position.quantity);
    ...
    if (!annualValue && Number.isFinite(Number(profile.annualPerShare)) && Number(profile.annualPerShare) > 0) {
      annualValue = convertCurrency(Number(profile.annualPerShare) * Number(row.position.quantity), currency, state.baseCurrency);
    }
    ...
    if (Number.isFinite(annualValue) && annualValue > 0) {
      payers.push({
        ...
        annualValue,
        yieldPercent: Number(profile.yieldPercent)
      });
    }
  }
...
async function fetchYahooDividendProfile(position) {
  ...
  const encoded = encodeURIComponent(yahooSymbol);
  const url = `https://query1.finance.yahoo.com/v8/finance/chart/${encoded}?range=5y&interval=1mo&events=div`;
  const data = await fetchJson(url, { fallbackProxy: true, timeoutMs: 9_000 });
  const error = data?.chart?.error;
...
function registerServiceWorker() {
  ...
  navigator.serviceWorker.register("./service-worker-v36.js", { scope: "./" }).catch((error) => {
