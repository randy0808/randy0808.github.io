...
async function loadFundamentals() {
  ...
  try {
    let rows = [];
    let source = "SEC companyfacts";
    try {
      rows = await fetchSecFundamentals(position);
    } catch (secError) {
      rows = await fetchYahooFundamentals(position);
    ...
    dom.fundamentalTableBody.innerHTML = emptyRow("SEC 財報資料暫時無法取得", 6);
  }
}
...
async function fetchYahooPriceHistory(target, rangeKey) {
  ...
  const url = `https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(yahooSymbol)}?range=${config.yahooRange}&interval=${config.yahooInterval}`;
...
async function fetchSecFundamentals(target) {
  ...
  const revenue = annualConcept(gaap, ["Revenues", "RevenueFromContractWithCustomerExcludingAssessedTax", "SalesRevenueNet"], (unit) => unit === "USD");
  const netIncome = annualConcept(gaap, ["NetIncomeLoss", "ProfitLoss"], (unit) => unit === "USD");
  ...
  const operatingCashFlow = annualConcept(gaap, ["NetCashProvidedByUsedInOperatingActivities"], (unit) => unit === "USD");
  ...
  const interestExpense = annualConcept(gaap, ["InterestExpenseNonOperating", "InterestExpense"], (unit) => unit === "USD");
  const years = [...new Set([
    ...eps.keys(),
    ...revenue.keys(),
    ...netIncome.keys(),
...
async function fetchYahooFundamentals(target) {
  ...
  const url = `https://query1.finance.yahoo.com/ws/fundamentals-timeseries/v1/finance/timeseries/${encodeURIComponent(symbol)}?symbol=${encodeURIComponent(symbol)}&type=${types.join(",")}&period1=${start}&period2=${end}`;
  const data = await fetchJson(url, { fallbackProxy: true, timeoutMs: 14_000 });
  const series = {};
...
function annualConcept(gaap, conceptNames, unitMatches) {
  for (const conceptName of conceptNames) {
    ...
    Object.entries(units).forEach(([unit, facts]) => {
      ...
      facts.forEach((fact) => {
        const year = Number(fact.fy || String(fact.frame || "").match(/CY(\d{4})/)?.[1]);
        ...
        if (!existing || String(fact.filed || "") > String(existing.filed || "")) {
          byYear.set(year, { value, filed: fact.filed });
        }
      });
    });
    if (byYear.size) return new Map([...byYear.entries()].map(([year, fact]) => [year, fact.value]));
  }
  return new Map();
}
...
function drawPriceChart(points) {
  ...
  const chart = { left: 68, right: width - 22, top: 24, bottom: height - 44 };
...
function normalizeSecTicker(symbol) {
  return String(symbol || "").trim().toUpperCase().replace(/\.(TW|TWO)$/i, "").replace(".", "-");
}
